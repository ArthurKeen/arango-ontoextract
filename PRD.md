...
6. [Core Features & Functional Requirements](#6-core-features--functional-requirements)
   ...
   - 6.21 Upper Ontology (System / Function / State Layer)
...
| FR-2.18 | Document-furniture rejection | Labels matching a structural token followed by a number (`Page 812`, `Figure 4`, `Table 12`, `Section 3.2`, `Appendix B`, `Step 7`) are artefacts **of** the document rather than concepts **described by** it, and are rejected at extraction. Existing filters do not catch them: the generic-term list is exact-match and the low-confidence rule applies only to single-word labels, so a two-token label at moderate confidence passes both. Rejection requires the trailing number, so legitimate concepts (`Page Layout`, `Table of Contents`) survive; both directions are test-pinned. Ontologies extracted before this rule are swept by migration, expiring matches temporally rather than deleting them so the removal is auditable and reversible, and reporting the affected count before any change is applied. |
...
| FR-4.16b | Multi-selection is visible and filterable in the sidebar | FR-4.16a syncs a SINGLE `selectedNodeKey` between canvas and sidebar. A multi-selection (FR-7.8.18) must sync too: every member is marked in the class tree, distinctly from the primary selection, and shift-clicking a row toggles membership so the tree is an input and not merely a mirror. **Marking alone is insufficient at scale and the requirement is not met by it:** with 96 classes selected out of 1688, scanning the tree for highlighted rows is no more usable than scanning the canvas. A **"Selected only"** filter is therefore required, reducing the tree to the current selection so it can be read, checked and acted on as a list. The selection count is shown alongside it. Rationale: a set action (FR-7.8.20) that reshapes twenty classes is only safe if the user can first see exactly which twenty. |
...
| FR-4.19 | Provenance opens on the strongest evidence | The provenance panel orders passages by how well they support the concept and opens on the strongest, rather than presenting an unordered list the reader must scan. Ranking uses the recorded `evidence_confidence` first, then span coverage, then position in the document. Where a class carries several evidence items the panel makes the ordering visible (a rank or score per passage) so a curator can see WHY a passage came first and disagree with it. In the document-level fallback (FR-4.7) no confidence exists, so ordering falls back to keyword density and MUST be labelled as heuristic — an ordering presented without its basis implies a precision the fallback does not have. Rationale: with a 700-page source, 'here are the passages' is only marginally better than 'here is the document' if the reader still has to find the one that matters. |
...
| FR-8b.8 | Property hierarchy (`rdfs:subPropertyOf`) | Object and datatype properties form a taxonomy via a `subproperty_of` edge collection mirroring `subclass_of` (temporal, org/ontology-scoped). The RDFS entailment rule holds: given `P rdfs:subPropertyOf P'` and `X P Y`, the assertion `X P' Y` is entailed, and queries may request the entailed closure. Export MUST emit `rdfs:subPropertyOf` — `owl_serializer` currently emits only `rdfs:subClassOf`, so a curated property hierarchy would be lost on round-trip. Entailment is what makes edge roll-up (FR-7.8.16) **sound rather than lossy**: a collapsed edge is a triple the ontology genuinely asserts, not an approximation of one. Required for sources with large relation vocabularies — the FinReflectKG catalog carries ~2,500 edge types, a list no per-edge-type view can render usefully. |
| FR-8b.9 | schema.org soft typing on import | Where a source types its properties with `schema:domainIncludes` / `schema:rangeIncludes` instead of `rdfs:domain` / `rdfs:range`, the importer MUST read the soft form as a **fallback** — never a supplement: a source stating both means the `rdfs:` one, and mixing them blurs two different semantics. Each resulting `rdfs_domain` / `rdfs_range_class` edge MUST record which predicate produced it. The two are not interchangeable: **`rdfs:domain` entails** (given `X P Y`, X is a member of the domain class) while **`schema:domainIncludes` deliberately does not** — it is a usage expectation, which is precisely why SOSA/SSN chose it, so the vocabulary can be reused without imposing inferences on the reuser. Multiple values mean *intersection* for `rdfs:domain` and *union* for `domainIncludes`, so a soft-typed property yields one edge per value. A soft value MUST NOT reach the property document's `range` field, which export turns into an `rdfs:range` triple — that would put an entailment into the exported ontology that the source never asserted. **Without this, SOSA/SSN imports as unconnected vertices**: it declares zero `rdfs:domain` and one `rdfs:range`, so all 36 of its object properties arrived with no class attachment and the Sensor→Observation→ObservableProperty→FeatureOfInterest pattern — the entire reason to adopt it — was invisible on the canvas (7 edges before, 58 after). |
...
| FR-14.8 | Derive SHACL shapes from constraints the system already holds | `export?format=shacl` emits shapes ONLY for rows whose `constraint_type` is `sh:NodeShape` / `sh:PropertyShape` — i.e. shapes that arrived as SHACL in an imported file (FR-14.3). Everything else is ignored, so an ontology that never imported a shapes graph exports a header and nothing more. **Measured 2026-08-31: all 1,327 constraint rows in the workspace are `owl:Restriction`; zero are SHACL, and SHACL export returns ~214 bytes of prefixes for every ontology.** The system therefore holds the material for shapes and declines to use it. Three sources exist today: (a) OWL restrictions imported from files — cardinality and value restrictions on a named class and property; (b) OWL restrictions the LLM extracts, which `ExtractedConstraint` already models and the prompt already requests — **but only in `tier1_technical`; `tier1_standard` and `tier1_visual_aware` never ask, which is why document extractions produce none**; (c) `ExtractedAttribute.range_datatype`, present on every extracted attribute, which is a `sh:datatype` for free, and `rdfs_domain` / `rdfs_range_class` edges, which give `sh:targetClass` / `sh:class`. Derivation MUST carry provenance (`derivedFrom` naming the constraint row) so a generated shape is never mistaken for one the source published. Deriving shapes does not execute them — validation remains FR-14.7. |
| FR-14.9 | A derived shape states validation, which the source did not | **This is the design decision FR-14.8 turns on, not an implementation detail.** OWL is open-world: `owl:minCardinality 1` licenses the inference that an instance has at least one such value; it does not say that an instance lacking one is wrong. SHACL is closed-world validation: `sh:minCount 1` says exactly that it is wrong. Translating the first into the second manufactures a claim the source ontology never made — the same class of error as FR-8b.9's soft-typing rule, where `schema:domainIncludes` must not become `rdfs:domain`. Therefore: a derived shape carries `sh:severity sh:Warning` by default, never `sh:Violation`, and records what it was derived from. A curator promotes it to `sh:Violation` deliberately, and that promotion is a recorded decision (§6.20 FR-20.3). Shapes derived from LLM-extracted restrictions carry the extraction confidence, because "the manual says SHALL" is a model's reading of prose, not an axiom the author wrote. Shapes that arrived AS SHACL keep the severity their author gave them — they were published as validation and need no reinterpretation. |
| FR-14.10 | SHACL is reachable from the UI | The export menu offers `turtle`, `jsonld` and `csv` (`frontend/src/app/library/page.tsx`). `shacl` is implemented in the backend and absent from the list, so the only way to obtain a shapes graph is to type the URL. It MUST appear alongside the others once FR-14.8 makes it non-empty; offering an export that is always empty is worse than not offering it. |
...
| FR-18.13 | A-box canvas rendering | Individuals are renderable on the graph canvas alongside their T-box classes: `rdf:type` edges connect an individual to its class and `individual_assertion` edges connect individuals to each other, each visually distinct from `subclass_of`/`has_property`. Instance display is opt-in and expanded per class ("show instances of X") rather than loading the whole A-box, so instance volume cannot swamp the T-box layout. Served by a dedicated read path so the latency-sensitive T-box effective-graph projection is unaffected. |

**Design note:** Builds on §6.2 (extraction), §6.7 (ER, for canonicalization), §6.14 (constraints, for validation), and Stream 16 (domain routing). **Resolves Stream 15 SO.4** ("A-box extraction — NEEDS DECISION") → decided: schema-grounded, EDC-style, per-domain, provenance-first. FR-18.13 closes the gap between this section (A-box data + curation) and §7.8 (canvas, authored pre-Stream-21 and T-box-only): FR-18.9's instance lens is a *list*, which structurally cannot show an individual positioned under its class or assertion edges between individuals.
...
| FR-19.4 | CQ scope propagation into extraction | The CQ term set (entities/relations the CQs reference) is applied as a required/priority signal across every source adapter. LLM-driven adapters (§6.2 unstructured and any future LLM-enriched graph path) inject it into the extraction prompt. Deterministic or externally delegated adapters without an AOE-owned prompt (relational and current graph-schema paths) apply it through adapter-appropriate post-extraction priority metadata/annotations on emitted classes and properties; when no requirements spec exists at extraction time, the adapter records the signal as not applicable and later coverage validation remains authoritative. Uniformity means a uniform use-case-prioritization effect, not an identical prompt-injection mechanism. |
...
**Description:** Two entities carrying the same attribute label (`Document.role` vs `Contact.role`) is mechanically detectable but not mechanically resolvable. Deciding that one means a document's kind and the other a person's job is a domain judgement, and the right answer is frequently neither existing name — `Contact.role` may be better as "job title", which no renaming rule would produce. AOE is already the curator-confirmed label store (status-gated, versioned, `label` + `description` per concept); this section adds the workflow that brings a collision to a curator, records what they decided, and — the overriding property — makes that decision **survive re-extraction**.

**Evidence.** Measured on a 71-question benchmark against the demo catalog (`nl2cnl/corpora/baselines/harmonization.json`), six label renames moved overall accuracy 60% → 69% and three-source accuracy 15% → 25%, beating four algorithmic attempts in the consuming repository (candidate ranking, join assembly, embedding-based concept resolution, clause pruning), two of which moved the number not at all. Identical across five runs. Three-source questions are 8 of 71, so 25% is two of them — a small sample, and the result argues for making curation cheap rather than the translator cleverer.

| ID | Requirement | Description |
|---|---|---|
| FR-20.1 | Collision work queue | Colliding labels are surfaced as a curator queue. Each item carries the label, the concepts carrying it, their source systems, and — where a producer supplies them — sample values from each, since seeing `"signal"`/`"qbr"` beside `"champion"`/`"exec"` resolves the question faster than any description. |
| FR-20.2 | Two producers | Collisions come from **local detection** (AOE groups live class/property labels across its own ontologies; same normalized label on two distinct `uri`s) and from **ingest** of an external report (the extractor, for same-document collisions; the merge layer, for cross-source ones). Generic labels (`id`, `name`, `type`, …) are excluded by default. |
| FR-20.3 | Resolution is a decision, not an edit | Resolving records the chosen label **per concept**, with who chose it and when. Partial resolution is first-class — renaming only one side is often correct. Re-deciding expires the prior decision rather than overwriting it, so the trail stays queryable; the audit trail is as much the value as the string. A collision may also be dismissed as acceptable. |
| FR-20.4 | Read-time overlay | Decisions live in their own store, joined to concepts by `uri`, and are merged over the extracted label on read — across entity lists, the class detail panel, the effective-graph canvas, and RDF export. Any read path that shows or exports a label MUST apply the overlay; one that skips it serves the pre-curation label. |
| FR-20.5 | Survival across re-extraction | A curated label MUST survive regeneration of the catalog from live sources. Extraction rebuilds entity `_key`s from the LLM's label and re-inserts with `overwrite=True`, so it reclaims any key it has used and a decision stored **on** the entity is destroyed; storing decisions where extraction never writes makes survival structural rather than a convention every future write path must remember. A curated label that a refresh discards is worse than none, because the improvement disappears without anyone noticing. |
| FR-20.6 | API | `GET/POST /ontology/lexicon/collisions` (+ `/detect`, `/ingest`, `/{key}/resolve`); `GET /ontology/lexicon/decisions` and `/decisions/history`. |

**Design note:** Complements §6.17 (alignment merges *concepts*; this disambiguates *names*) and feeds the downstream lexicon contract that consuming translators read. Explicitly **not** in scope: merging duplicated concepts, and the deeper modelling debt behind it — an `Opportunity` carrying its own `renewalDate` rather than referring to the contract's, seats counted four ways, every entity carrying `accountId` so any join stays expressible. That work is worth scoping on this evidence, but it should be scoped, not assumed.

---

### 6.21 Upper Ontology (System / Function / State Layer)

**Description:** Extraction currently produces domain classes with no notion of *what kind of thing* each one is. The consequence is not cosmetic: classes that are obviously related to a human reader are left unconnected because nothing in the system knows how their kinds relate. Observed in the JLR extraction — `Vehicle Security` (whose own description says it reports open/closed status, making it a **state**), `Vehicle Security System` (a **subsystem**), and `Vehicle Security Feature` (a **function exposed to a user**) were extracted as three mutually unrelated classes. They are not three mistakes; they are three different kinds of thing that the extractor had no vocabulary to distinguish.

An upper ontology supplies that vocabulary — a small, curated layer of kinds and the relations that hold between them — so that connections follow from typing rather than from the LLM happening to assert them.

**Distinct from §6.8b.** The foundation layer covers the *formal* metamodel (`owl:Class`, `rdfs:subClassOf`, `rdfs:domain`). It says nothing about systems having functions. Both layers are required; the upper ontology sits above the formal one and below the domain.

| ID | Requirement | Description |
|---|---|---|
| FR-21.1 | Upper-ontology layer | A curated ontology of kinds — minimally System, Subsystem, Component, Function, Feature, State, Event, Procedure, Actor — with the relations between them: a system *has* subsystems (transitive), *has* functions, *has* state, *has* purpose; a feature *is* a function exposed to a user; a system *monitors* and *triggers* events; an event is instantaneous or has duration. Stored as a first-class ontology that domain ontologies pull in through the existing `owl:imports` mechanism (§6.15), so it is versioned, curatable and releasable like any other — never hard-coded. |
| FR-21.2 | Typed extraction | Extraction assigns each domain class a kind from the upper layer, carried as evidence-backed like any other assertion (FR-4.7) rather than asserted without support. The schema-retrieval prompt (FR-18.2) includes the upper layer so the LLM types against a fixed vocabulary instead of inventing one. |
| FR-21.3 | Untyped and orphan detection | A class with no upper-ontology kind is a reportable validation failure, not an invisible gap. Likewise a class whose kind implies a relation it does not have — a Feature with no system exposing it, a State with nothing it is the state of. This is the diagnostic payoff: today an orphan is indistinguishable from a correctly isolated concept. |
| FR-21.4 | Kind-driven relation inference | Where typing licenses a relation that the extraction did not assert, the system **proposes** it for curation rather than asserting it (§6.16 belief revision). Naming alone is suggestive but not sufficient evidence — `Vehicle Security Feature` is very likely a feature of the vehicle security system, but "very likely" is a candidate, not a fact. |
| FR-21.5 | Naming guidance from kind | Where a class's kind contradicts its label, the mismatch is surfaced for curation — a class typed as State but named as though it were a system (`Vehicle Security`, better `Vehicle Security Status`) is a rename candidate. Feeds the curated-lexicon decision store (§6.20) so the rename is recorded with attribution and survives re-extraction (FR-20.5). |
| FR-21.6 | Relation vocabulary between kinds | The upper layer defines the RELATIONS as well as the kinds, each with a domain and range stated in terms of those kinds, so an assertion can be checked rather than merely recorded. Minimally: `partOf` (transitive, Component→Subsystem→System — *Tyre partOf Tyre System partOf Vehicle*); `hasAttribute` (Thing→Attribute — a Speed Rating attaches to a Tyre without claiming to be one); `documentedBy` (Thing→Document — manufacturer instructions are *about* a tyre); `hasFunction` (System→Function); `exposes` (System→Feature, a function made available to a user); `hasState` (System→State); `monitors` and `triggers` (System→Event). Every relation carries an inverse where one is meaningful, so the graph can be traversed from either end. Extraction selects from this vocabulary instead of defaulting to `rdfs:subClassOf`, which is the actual defect: with exactly one hierarchical relation available, every association the model senses is flattened into subsumption. |
| FR-21.7 | Subsumption validation (the is-a test) | Every `rdfs:subClassOf` assertion MUST survive the question *"is every X a Y?"*, checked against the kinds from FR-21.2 and the relation vocabulary from FR-21.6. A subclass edge whose child and parent have incompatible kinds is rejected and re-proposed as the relation the kinds do license. The four cases observed in the JLR extraction are the acceptance criteria, and all four must be caught: **`Tyre subClassOf Vehicle`** — a tyre is not a vehicle; Component→System licenses `partOf`, better still via an intervening Tyre System. **`Speed Rating subClassOf Tyre`** — a speed rating is not a tyre; Attribute→Component licenses `hasAttribute`. **`Tyre Manufacturer Instructions subClassOf Tyre`** — a document is not the thing it describes; Document→Component licenses `documentedBy`. **`Tyre` / `Tyre Type`** — not a subsumption error but a duplicate pair, routed to merge (§6.7) rather than left as two concepts. Rejections are PROPOSALS for curation, never silent rewrites (FR-21.4): the system says which relation the kinds permit and why the asserted one fails, and a curator decides. Existing ontologies are validated retrospectively, so the report is available before any re-extraction. |

**Measured evidence (2026-08-23).** Screened all 753 `subclass_of` edges in the 1688-class JLR extraction. 205 (27%) share a head noun with their parent and are plausible is-a links. 116 (15%) have a child whose head noun is a document, attribute or process word — high-confidence misuse. A hand-judged random sample of 25 from the remaining 432 found 16 failing the is-a test (64%, 95% CI 45–83%), giving an estimated **~390 of 753 subsumption edges (52%, range 41–63%) that are not subsumption at all**. The dominant pattern by far is **part-of masquerading as is-a** — *Airbag subClassOf Supplementary Restraint System*, *Roof Rack subClassOf Vehicle*, *Smart Key Component subClassOf Smart Key System* — which is what FR-21.6's transitive `partOf` exists to absorb. Also present: kind confusion (*Safety Feature subClassOf Vehicle Component* — a feature is not a component) and outright category errors (*Garage Door Opener subClassOf Owner's Handbook*). Method is a lexical screen plus a manual sample, not a semantic oracle; it is an estimate with a stated interval, not a count.

**Design note:** The source of the upper layer is **unresolved — see Q15**. It is a modelling decision (hand-author / adopt a standard / align to one) and blocks this entire section; the requirements above are deliberately written to be independent of which is chosen.
...
| FR-7.8.12 | Light/dark colour mode | Every dashboard carries a theme toggle in its header. The choice applies app-wide, persists per device, and is resolved **before first paint** so no page flashes the wrong mode; absent a stored choice the OS preference is followed, defaulting to light. Themes are two palettes over one set of CSS custom properties (`html[data-theme="dark"]` remaps the neutral tokens) rather than per-component variants, so the Arango brand greens and the error red are **identical in both modes** and only the neutrals invert. The graph workspace is explicitly **exempt**: its canvas is hand-painted dark and its edge palette is tuned for separation on a dark background, so it renders identically in both modes — this is a deliberate choice, not an unconverted screen. |
| FR-7.8.13 | Search reaches class and property labels | Asset-explorer search matches class and property labels *within* each ontology, not only document filenames and ontology names, and attributes each hit to its owning ontology. A canvas-scoped search bar searches the open ontology and jumps to plus focuses a hit. Name-only search is why querying a class name returns nothing today. |
| FR-7.8.14 | Pending work is visible without opening a menu | Assets carrying unreviewed work — revision suggestions, competency-question proposals, curation candidates — show a count badge on their explorer row. Context-menu-primary navigation requires explicit discoverability mitigation (§7.8 rule 20); a capability reachable only by right-clicking and already knowing it exists is functionally absent. |
| FR-7.8.15 | Focus mode with hop radius | Selecting a node dims the graph beyond a configurable hop radius (1 / 2 / 3 / all) to a low opacity rather than hiding it, so local structure becomes legible while context survives. Selecting an edge highlights its two endpoints. Focus is a **paint** operation and MUST NOT relayout — the topology fingerprint is unchanged, per the lens rule. Dimming the complement is the mechanism, not emphasising the selection: at 667 classes a single highlighted node is one lit circle among hundreds and is not findable. |
| FR-7.8.16 | Hierarchy roll-up and tree view | Classes collapse to their ancestor at depth *k* along `subclass_of`, and relations to their parent property along `subproperty_of` (FR-8b.8), each collapsed node carrying a member count. The grouping is the ontology's own hierarchy and is therefore explainable to a reviewer; community-detection clustering is **explicitly rejected** — its groups are artefacts of edge density, unstable across runs, and not defensible as ontology structure. A dedicated indented-tree / dendrogram canvas view renders `subClassOf` and partonomy, for which a force-directed layout is the wrong instrument. The UML box-and-arrow view is fed from a focus neighbourhood (FR-7.8.15) rather than the whole graph — it fails at scale because it is unfiltered, not because the notation is wrong. |
| FR-7.8.17 | Hide and isolate, with undo | Beyond dimming (FR-7.8.15), the canvas offers destructive-looking but non-destructive view actions from the right-click menu: **Hide other nodes** (keep the selection and its focus neighbourhood, hide the rest) and **Hide selected**. Both operate on a *view* set — nothing is expired, deleted or written; reloading the ontology restores everything. Hiding MUST preserve the positions of surviving nodes so the curator does not lose their bearings, and a persistent affordance MUST show how many nodes are hidden with a one-click **Show all** to reverse it. **Undo is the requirement, not a nicety:** the ArangoDB Graph Visualizer offers remove-others / remove-selected with no way back, which turns an exploratory action into a dead end and trains users not to touch it. Dimming remains the default because it preserves context; hiding is for when the surrounding graph is so dense that dimmed nodes still occlude the neighbourhood being read. |
| FR-7.8.18 | Canvas selection model | Selection is direct-manipulable, not a one-way trap. **Clicking empty canvas clears the selection** — today the only way out of a selection is to select something else. **Shift-click adds** a node to the selection and **shift-clicking a selected node removes it**, so a multi-selection can be built and unpicked without starting over. **Lasso**: holding a modifier and dragging draws a rectangle that selects every node inside it, for grabbing a cluster that no single click can express. Multi-selection drives the same downstream actions as single selection (hide, focus, curation) so the two are not separate feature sets. |
| FR-7.8.19 | Detail panel opens on request, not on selection | Left-click selects and nothing more; the class/edge detail panel opens from an explicit **View Details** in the right-click menu, for nodes AND edges. Auto-opening on every selection makes exploratory clicking tedious — the panel occludes the canvas and must be dismissed before the next click — and it conflicts with FR-7.8.18, where selection becomes a frequent, low-cost gesture used for hiding, focusing and lassoing rather than an implicit request to read. Selection must stay cheap; reading is the deliberate act. |
| FR-7.8.20 | Set actions on a multi-selection | Right-clicking a node that is **inside** the current multi-selection opens a SET menu acting on all of it; right-clicking a node **outside** it collapses the selection to that node and opens the normal single-node menu. The second half is a safety property, not a nicety: without it a right-click on an unrelated node silently applies to N invisible others. The set menu states its size in every destructive label ("Reject these 20"), and inherently singular actions (View Details, Version History, Provenance, Copy URI) remain, scoped to the clicked node and labelled with it — a large selection must not cost the user the ability to inspect the node under the cursor. **Actions.** *Curation:* approve / reject the whole set, routed through the existing batch-decision path so every member gets an attributed, temporal decision. *View:* hide the set, hide everything else, clear the selection. *Structural (the reason multi-select is worth building):* **introduce a superclass** — create one new class and make every selected class `rdfs:subClassOf` it, which is the core taxonomy-building move on a flat extraction; and **set parent** — make every selected class a subclass of an existing one. Both reuse the atomic, cycle-rejecting reparent path, and both report per-class outcomes rather than failing silently in the middle, so a partial result is visible and recoverable. Structural edits are temporal like any other, so they are undoable via time travel. **Rationale.** A 667-class extraction is flat: the single most valuable curation act is selecting a cluster of siblings and giving them a parent. Doing that one class at a time is why the taxonomy never gets built. |
| FR-7.8.21 | Structural set actions are undoable | A set action that reshapes the taxonomy (FR-7.8.20 introduce-superclass, set-parent, merge) MUST return the information needed to reverse it and MUST offer a one-click undo for the remainder of the session. For a reparent that is each class's PREVIOUS parent, which the reparent path already computes and currently discards. Undo restores those parents through the same cycle-checked path, and reports per-class outcomes like the forward operation. **Everything is already temporal and therefore recoverable in principle — but recoverable is not undoable:** there is no revert operation for edges (only `revert_class`), so reversing a twenty-class reparent by hand requires knowing twenty former parents that the UI never showed. This is the same standard FR-7.8.17 sets for hiding, and it matters more here: hiding is visibly reversible, whereas a bulk reparent silently reshapes a hierarchy in a way that is hard even to notice later. A created superclass left empty by an undo is reported, not auto-deleted — deleting a class the user may have named deliberately is not the system's call. |
| FR-7.8.22 | Merge a selection into one class | A multi-selection can be merged into a single surviving class from the set menu (FR-7.8.20). The curator chooses which class SURVIVES — never the system, since picking a winner by confidence or label length would silently discard the name a domain expert would have kept. The dialog names the survivor and lists exactly what is being folded into it. Source classes are temporally expired and their edges re-pointed at the survivor, so no relationship is orphaned by the merge. **Merge is the one destructive set action without a one-click undo** (FR-7.8.21 covers reparenting): sources remain queryable as-of a prior time, but there is no un-merge operation, so the dialog MUST say so plainly rather than implying the same reversibility as its neighbours in the menu. Motivation: the JLR extraction produced `Tyre` and `Tyre Type` as separate concepts, alongside many near-duplicates; merging duplicates is the cheapest quality win available and needs no modelling decision, so it is not blocked on the upper ontology. |
| FR-7.8.23 | Mirrored relations are drawn once | Where two object properties are declared `owl:inverseOf` each other, the canvas MUST draw the relation **once** rather than as two arrows between the same pair of classes: the pair states one fact in both directions, and drawing both doubles the edge count while adding nothing. The importer MUST materialise the **symmetric mirror**, because a source states the axiom once (SOSA writes `isFeatureOfInterestOf owl:inverseOf hasFeatureOfInterest` and leaves the reverse implicit) and every consumer would otherwise re-derive the symmetry independently. The surviving direction MUST be chosen **deterministically** — an unstable choice makes the canvas flicker between renders. Which direction survives is presentational, not semantic: prefer the active label over the passive `isXOf` / `madeByX` form, falling back to a stable identifier comparison when the two cannot be told apart. The dropped reading MUST remain nameable on the surviving edge rather than being discarded. An edge MUST NOT be collapsed against a partner that is not itself drawable, or the relation disappears entirely. Measured: SOSA/SSN 58 canvas edges → 36, with 26 of its 36 object properties carrying an inverse. |
...
| Q15 | Upper-Ontology Source | High | §6.21 commits to an upper ontology (System / Subsystem / Function / Feature / State / Event) but not to where it comes from. Options: (a) **hand-author** ~12 classes specific to this domain; (b) **adopt** an existing standard — ISO 15926 (process industry), IEC 81346 (systems/installations reference designation), SSN/SOSA (sensing, observation, actuation) for the event and monitoring parts, or BFO for maximal rigour; (c) **align, don't derive** — hand-author a small layer and declare alignments (`owl:equivalentClass` / `rdfs:subClassOf`) to a standard, so interoperability is available without inheriting the standard's full commitments. **Evidence added 2026-08-23:** review of the JLR extraction found `subClassOf` used as a catch-all for any association — `Tyre subClassOf Vehicle`, `Speed Rating subClassOf Tyre`, `Tyre Manufacturer Instructions subClassOf Tyre`. The fix needs `partOf` (transitive), `hasAttribute` and `documentedBy` with real domain/range constraints (FR-21.6/21.7). These are standard, well-specified relations with known pitfalls — transitive part-of in particular is easy to get subtly wrong — which strengthens the case for aligning to a standard rather than hand-authoring their semantics from scratch. **Recommendation:** (c). A 700-page owner's manual does not need BFO's continuant/occurrent split to distinguish a subsystem from a feature, and standards bring modelling baggage that a curator must then reconcile on every extraction; but an isolated bespoke layer forfeits interoperability, which alignment recovers cheaply. **Owner:** the ontology lead — this is a modelling decision, not an engineering one, and it blocks all of §6.21. |
