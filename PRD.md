...
| FR-4.12c | Layout algorithm selection | The canvas toolbar or context menu provides a layout selector with at least: **ForceAtlas2** (default, force-directed), **Circular** (nodes on a circle), **Random** (reset positions). Layout computation runs on the graphology graph and Sigma re-renders. Disconnected components should be spread apart so they don't overlap. |
| FR-4.12d | Node shape variety | Sigma.js renders circles by default (WebGL `NodeCircleProgram`). For OWL type distinction, different node types MAY use different visual treatments: size variation by degree, color by type (already implemented via lenses), and border styling for state (staging vs approved). Custom WebGL node programs (e.g., `NodePointProgram` for smaller auxiliary nodes) can be registered for future use. |
