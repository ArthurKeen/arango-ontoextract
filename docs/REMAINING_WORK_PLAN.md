...
| # | Task | Type | Status | Description |
|---|------|------|--------|-------------|
| S.1 | Wire `arangodb-schema-analyzer` integration | Backend | **DONE (pre-existing)** | `_try_import_schema_mapper()` + `_run_schema_mapper_extract()` -- graceful degradation when the optional library isn't installed (it isn't, by default). |
...
| RS.5 | CSI v1 document import | Backend | **DONE 2026-09-14** | PRD FR-9.16. `backend/app/services/csi_import.py` (validate → `build_csi_owl` → `import_from_file` → per-class provenance stamping); API sub-router `app/api/ontology/schema_csi.py` (`POST .../schema/csi/preview` read-only, `POST .../schema/csi/import`; `400` unreadable document, `500` otherwise); MCP tools `preview_csi_document` + `import_csi_document` (`app/mcp/tools/csi.py`). Ingests what `r2g export-csi` or `arangodb-schema-analyzer` wrote, unchanged, and **records the analyzer's type-detection answer** (per-entity `arangoPhysicalMapping.style`) instead of re-detecting. This is AOE's seam to the contextual-data-fabric structured path; AOE's own LPG detector (Stream 5 / FR-9.14) is slated to be retired in favour of consuming ASA 0.14.0's `LABEL` / `GENERIC_WITH_TYPE` answer through this import — not yet done. |
...
**Clarification — the structured/unstructured "split question" is answerable YES, with a boundary (restated 2026-09-15).** AOE has structured→ontology paths of its own: **Stream 18** (relational SQL → OWL/SHACL, AOE's own product feature; `relational-schema-analyzer` is a read-only `PhysicalSchema` introspector) and **Stream 5** (ArangoDB schema → OWL/SHACL). But AOE is **not** contextual-data-fabric's structured-schema pipeline: CDF's relational CSI + R2RML are produced by **r2g via `relational-schema-analyzer`**, and CDF's Arango CSI by **`arangodb-schema-analyzer`**. AOE's role toward CDF is **curation via CSI import** (RS.5 — `csi_import.py`, `POST .../schema/csi/preview` + `/import`, MCP `preview_csi_document` / `import_csi_document`): AOE reads the CSI those tools emit, records their type-detection answer rather than re-detecting, and in the target design its curated output is what CDF's catalog ingests. So AOE is **not** unstructured-only; RE-2 (alignment) is the genuinely missing piece, not the structured path.
