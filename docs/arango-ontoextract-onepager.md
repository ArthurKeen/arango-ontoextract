...
- **Reconcile many sources into one master.** Independently-built ontologies are aligned into a governed master: embedding retrieval proposes correspondences, an LLM adjudicates only the uncertain middle, a classical-anchor ensemble catches hallucinated matches, and incoherent merges are minimally repaired — every removal reported, the whole thing reversible.
- **Schema *and* instances.** Beyond the T-box (the schema), AOE extracts the A-box — the actual **individuals** and their relationships — grounded in the schema, traceable to the exact source span, and curated over the same temporal layer.
...
- **Bootstrap from a database.** Reverse-engineer an ontology directly from a schema you already run — ArangoDB (collections to classes, edges to object properties, constraints to SHACL; labeled-property-graph shapes where types live in a field are handled too) or a relational (SQL) database — with full provenance back to the source table or collection. Type detection is converging on `arangodb-schema-analyzer` 0.14.0, whose answer AOE consumes via CSI import rather than re-deriving.
...
- **See what most graph tools don't draw.** Published vocabularies state much of their structure as OWL *restrictions* rather than plain properties — invisible to viewers that only draw properties, which makes well-connected classes look isolated. AOE renders restriction edges as **a view you switch on**: on to audit what an imported standard actually says, off for the plain property graph, and switching never re-runs the layout.
- **And those constraints become runnable validation — carefully.** The same restrictions derive **SHACL** shapes. But OWL is open-world and SHACL is closed-world, so a derived shape is always `sh:Warning`, never `sh:Violation`, and carries provenance back to the row it came from; promoting one is a curator's recorded decision. The system may reorganize what it was told and may never upgrade it into a stronger claim.
